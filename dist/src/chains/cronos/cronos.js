...
class Cronos extends ethereum_base_1.EthereumBase {
    constructor(network) {
        ...
        super('cronos', config.network.chainID, config.network.nodeURL, config.network.tokenListSource, config.network.tokenListType, config.manualGasPrice, config.gasLimitTransaction, config_manager_v2_1.ConfigManagerV2.getInstance().get('server.nonceDbPath'), config_manager_v2_1.ConfigManagerV2.getInstance().get('server.transactionDbPath'));
    ...
    getSpender(reqSpender) {
        ...
        if (reqSpender === 'mad_meerkat') {
            ...
            spender = openocean_config_1.OpenoceanConfig.config.routerAddress('cronos', this._chain);
        }
        else {
            spender = reqSpender;
        }
