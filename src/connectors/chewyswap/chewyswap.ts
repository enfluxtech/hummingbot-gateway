...
import { ChewyswapConfig } from './chewyswap.config';
import routerAbi from './chewyswap_router.json';
...
export class Chewyswap implements Uniswapish {
  private static _instances: { [name: string]: Chewyswap };
  ...
  private constructor(chain: string, network: string) {
    const config = ChewyswapConfig.config;
    if (chain === 'shibarium') {
  ...
  public static getInstance(chain: string, network: string): Chewyswap {
    if (Chewyswap._instances === undefined) {
      Chewyswap._instances = {};
    }
    if (!(chain + network in Chewyswap._instances)) {
      Chewyswap._instances[chain + network] = new Chewyswap(chain, network);
    }

    return Chewyswap._instances[chain + network];
  }
  ...
  getSlippagePercentage(): Percent {
    const allowedSlippage = ChewyswapConfig.config.allowedSlippage;
