...
describe('Test logger', () => {
    it('updateLoggerToStdout works', (done) => {
        config_manager_v2_1.ConfigManagerV2.getInstance().set('server.logToStdOut', true);
        ...
        config_manager_v2_1.ConfigManagerV2.getInstance().set('server.logToStdOut', false);
        (0, logger_1.updateLoggerToStdout)();
        done();
    });
    it('test telemetry transport can be added', () => {
        const initTransports = logger_1.logger.transports.length;
        if (!config_manager_v2_1.ConfigManagerV2.getInstance().get('server.telemetry_enabled')) {
            config_manager_v2_1.ConfigManagerV2.getInstance().set('server.telemetry_enabled', true);
            (0, logger_1.telemetry)();
            config_manager_v2_1.ConfigManagerV2.getInstance().set('server.telemetry_enabled', false);
