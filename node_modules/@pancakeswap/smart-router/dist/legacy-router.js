...
chunkNYOHPJYN_js.__export(legacyRouter_exports, {
...
async function getPairs(currencyPairs, { provider, chainId }) {
  const tokens = currencyPairs.map(([currencyA, currencyB]) => [
    chunkNYOHPJYN_js.wrappedCurrency(currencyA, chainId),
    chunkNYOHPJYN_js.wrappedCurrency(currencyB, chainId)
  ]);
...
async function getStableSwapOutputAmount(pair, inputAmount, { provider }) {
  const wrappedInputAmount = chunkNYOHPJYN_js.wrappedCurrencyAmount(inputAmount, inputAmount.currency.chainId);
...
function getStableSwapFeeCall(pair, inputAmount) {
  ...
  const wrappedInputAmount = chunkNYOHPJYN_js.wrappedCurrencyAmount(inputAmount, chainId);
  if (!wrappedInputAmount) {
    throw new Error(`No wrapped token amount found for input amount: ${inputAmount.currency.name}`);
  }
  ...
  return {
    ...
    address: pair.infoStableSwapAddress || chunkNYOHPJYN_js.STABLE_SWAP_INFO_ADDRESS[chainId],
...
function getStableSwapPairs(chainId) {
  const pools = chunkNYOHPJYN_js.getStableSwapPools(chainId);
  return pools.map(
    ({
      token,
      quoteToken,
      ...
      const token0 = tokenLists.deserializeToken(token);
      const token1 = tokenLists.deserializeToken(quoteToken);
...
var stableSwapPairsByChainId = fromPairs___default.default(
  chunkNYOHPJYN_js.STABLE_SUPPORTED_CHAIN_IDS.map((chainId) => [chainId, getStableSwapPairs(chainId)])
);
...
function createGetBestTradeFromV2(tradeType) {
  ...
  return async function bestTradeFromV2(amountIn, output, options) {
    ...
    for (let i = 1; i <= maxHops; i++) {
      ...
      if (sdk.isTradeBetter(bestTradeSoFar, currentTrade, chunkNYOHPJYN_js.BETTER_TRADE_LESS_HOPS_THRESHOLD)) {
