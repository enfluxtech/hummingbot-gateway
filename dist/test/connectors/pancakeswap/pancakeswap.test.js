...
const patchFetchPairData = () => {
    (0, patch_1.patch)(sdk_1.Fetcher, 'fetchPairData', () => {
        return new sdk_1.Pair(sdk_1.CurrencyAmount.fromRawAmount(WBNB, '2000000000000000000'), sdk_1.CurrencyAmount.fromRawAmount(DAI, '1000000000000000000'));
    });
};
const patchTrade = (key, error) => {
    (0, patch_1.patch)(sdk_1.Trade, key, () => {
        if (error)
            return [];
        const WBNB_DAI = new sdk_1.Pair(sdk_1.CurrencyAmount.fromRawAmount(WBNB, '2000000000000000000'), sdk_1.CurrencyAmount.fromRawAmount(DAI, '1000000000000000000'));
        const DAI_TO_WBNB = new sdk_1.Route([WBNB_DAI], DAI, WBNB);
        return [
            new sdk_1.Trade(DAI_TO_WBNB, sdk_1.CurrencyAmount.fromRawAmount(DAI, '1000000000000000'), sdk_1.TradeType.EXACT_INPUT),
        ];
    });
};
