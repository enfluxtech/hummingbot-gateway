...
class Polygon extends ethereum_base_1.EthereumBase {
    constructor(network) {
        ...
        super('polygon', config.network.chainID, config.network.nodeURL, config.network.tokenListSource, config.network.tokenListType, config.manualGasPrice, config.gasLimitTransaction, config_manager_v2_1.ConfigManagerV2.getInstance().get('server.nonceDbPath'), config_manager_v2_1.ConfigManagerV2.getInstance().get('server.transactionDbPath'));
    ...
    getSpender(reqSpender) {
        ...
        if (reqSpender === 'uniswap') {
            ...
            spender = sushiswap_config_1.SushiswapConfig.config.sushiswapRouterAddress('polygon', this._chain);
        }
        else if (reqSpender === 'openocean') {
            spender = openocean_config_1.OpenoceanConfig.config.routerAddress('polygon', this._chain);
        }
        else if (reqSpender === 'curve') {
            const curve = curve_1.Curve.getInstance('ethereum', this._chain);
            if (!curve.ready()) {
                curve.init();
