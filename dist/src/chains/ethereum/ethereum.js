...
const evm_controllers_1 = require("./evm.controllers");
...
class Ethereum extends ethereum_base_1.EthereumBase {
    constructor(network) {
        ...
        super('ethereum', config.network.chainID, config.network.nodeURL, config.network.tokenListSource, config.network.tokenListType, config.manualGasPrice, config.gasLimitTransaction, config_manager_v2_1.ConfigManagerV2.getInstance().get('server.nonceDbPath'), config_manager_v2_1.ConfigManagerV2.getInstance().get('server.transactionDbPath'));
        this._chain = network;
