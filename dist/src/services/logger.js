...
const getLogPath = () => {
    let logPath = config_manager_v2_1.ConfigManagerV2.getInstance().get('server.logPath');
...
const updateLoggerToStdout = () => {
    config_manager_v2_1.ConfigManagerV2.getInstance().get('server.logToStdOut') === true
        ? exports.logger.add(toStdout)
        : exports.logger.remove(toStdout);
};
exports.updateLoggerToStdout = updateLoggerToStdout;
const telemetry = () => {
    config_manager_v2_1.ConfigManagerV2.getInstance().get('server.telemetry_enabled') === true
