...
var chunkNYOHPJYN_js = require('./chunk-NYOHPJYN.js');
...
chunkNYOHPJYN_js.__export(stableSwap_exports, {
  getD: () => getD,
...
chunkNYOHPJYN_js.__export(smartRouter_exports, {
...
function encodeMixedRouteToPath(route, exactOutput) {
  ...
  const { path, types } = route.pools.reduce(
    ({ inputToken, path: path2, types: types2 }, pool, index) => {
      ...
      const fee = isV3Pool(pool) ? pool.fee : chunkNYOHPJYN_js.V2_FEE_PATH_PLACEHOLDER;
      if (index === 0) {
        return {
...
function getUsdGasToken(chainId) {
  return chunkNYOHPJYN_js.usdGasTokensByChain[chainId]?.[0] ?? null;
}
...
chunkNYOHPJYN_js.__export(transformer_exports, {
...
var getCheckAgainstBaseTokens = memoize__default.default((currencyA, currencyB) => {
  ...
  const [tokenA, tokenB] = chainId ? [chunkNYOHPJYN_js.wrappedCurrency(currencyA, chainId), chunkNYOHPJYN_js.wrappedCurrency(currencyB, chainId)] : [void 0, void 0];
  if (!tokenA || !tokenB) {
    return [];
  }
  const common = chunkNYOHPJYN_js.BASES_TO_CHECK_TRADES_AGAINST[chainId] ?? [];
  const additionalA = tokenA ? chunkNYOHPJYN_js.ADDITIONAL_BASES[chainId]?.[tokenA.address] ?? [] : [];
  const additionalB = tokenB ? chunkNYOHPJYN_js.ADDITIONAL_BASES[chainId]?.[tokenB.address] ?? [] : [];
...
var getPairCombinations = memoize__default.default((currencyA, currencyB) => {
  ...
  return [
    ...
    if (!chainId)
      return true;
    const customBases = chunkNYOHPJYN_js.CUSTOM_BASES[chainId];
...
function getBestSwapRouteBy(tradeType, percentToQuotes, percents, chainId, by, { maxSplits = 4, minSplits = 0 }) {
  ...
  if (!chunkNYOHPJYN_js.usdGasTokensByChain[chainId] || !chunkNYOHPJYN_js.usdGasTokensByChain[chainId][0]) {
    throw new Error(`Could not find a USD token for computing gas costs on ${chainId}`);
  }
  const usdToken = chunkNYOHPJYN_js.usdGasTokensByChain[chainId][0];
    ...
    for (const pool of pools) {
      ...
      if (isV2Pool(pool)) {
        if (!poolTypeSet.has(type)) {
          baseGasUse += chunkNYOHPJYN_js.BASE_SWAP_COST_V2;
          poolTypeSet.add(type);
          continue;
        }
        baseGasUse += chunkNYOHPJYN_js.COST_PER_EXTRA_HOP_V2;
        continue;
      }
      if (isV3Pool(pool)) {
        if (!poolTypeSet.has(type)) {
          baseGasUse += chunkNYOHPJYN_js.BASE_SWAP_COST_V3(chainId);
          poolTypeSet.add(type);
        }
        baseGasUse += chunkNYOHPJYN_js.COST_PER_HOP_V3(chainId);
        continue;
      }
      if (isStablePool(pool)) {
        if (!poolTypeSet.has(type)) {
          baseGasUse += chunkNYOHPJYN_js.BASE_SWAP_COST_STABLE_SWAP;
          poolTypeSet.add(type);
          continue;
        }
        baseGasUse += chunkNYOHPJYN_js.COST_PER_EXTRA_HOP_STABLE_SWAP;
        continue;
      }
    }
    const tickGasUse = chunkNYOHPJYN_js.COST_PER_INIT_TICK(chainId) * totalInitializedTicksCrossed;
    const uninitializedTickGasUse = chunkNYOHPJYN_js.COST_PER_UNINIT_TICK * 0n;
...
var V3_TOKEN_POOL_SELECTOR_CONFIG = {
  [chains.ChainId.BSC]: {
    [tokens.bscTokens.ankr.address]: {
      topNTokenInOut: 4
    },
    [tokens.bscTokens.ankrbnb.address]: {
      topNTokenInOut: 4
    },
    [tokens.bscTokens.ankrETH.address]: {
      topNTokenInOut: 4
    },
    [tokens.bscTokens.wbeth.address]: {
      topNSecondHop: 3
    }
  }
};
...
var getV2PoolsOnChain = createOnChainPoolFactory({
  abi: chunkNYOHPJYN_js.pancakePairABI,
...
var getStablePoolsOnChain = createOnChainPoolFactory({
  abi: chunkNYOHPJYN_js.stableSwapPairABI,
  getPossiblePoolMetas: ([currencyA, currencyB]) => {
    const poolConfigs = chunkNYOHPJYN_js.getStableSwapPools(currencyA.chainId);
...
var createGetTokenPriceFromLlmaWithCache = ({
  ...
  return async ({ addresses, chainId }) => {
    if (!chainId || !chains.getLlamaChainName(chainId)) {
    ...
    const list = addressesToFetch.map((address) => `${chains.getLlamaChainName(chainId)}:${address.toLocaleLowerCase()}`).join(",");
...
function poolSelectorFactory({
  ...
  return function tvlSelector(currencyA, currencyB, unorderedPoolsWithTvl) {
    ...
    const baseTokens = chunkNYOHPJYN_js.BASES_TO_CHECK_TRADES_AGAINST[chainId] ?? [];
...
var SUCCESS_RATE_CONFIG = {
  [chains.ChainId.BSC_TESTNET]: 0.1,
  [chains.ChainId.BSC]: 0.1,
  [chains.ChainId.ETHEREUM]: 0.1,
  [chains.ChainId.GOERLI]: 0.1,
  [chains.ChainId.ARBITRUM_ONE]: 0.1,
  [chains.ChainId.ARBITRUM_GOERLI]: 0.1,
  [chains.ChainId.POLYGON_ZKEVM]: 0.01,
  [chains.ChainId.POLYGON_ZKEVM_TESTNET]: 0,
  [chains.ChainId.ZKSYNC]: 0.2,
  [chains.ChainId.ZKSYNC_TESTNET]: 0.1,
  [chains.ChainId.LINEA]: 0.1,
  [chains.ChainId.LINEA_TESTNET]: 0.1,
  [chains.ChainId.OPBNB]: 0.1,
  [chains.ChainId.OPBNB_TESTNET]: 0.1,
  [chains.ChainId.BASE]: 0.1,
  [chains.ChainId.BASE_TESTNET]: 0.1,
  [chains.ChainId.SCROLL_SEPOLIA]: 0.1
};
...
var retryControllerFactory = ({ retries }) => {
  ...
  return {
    shouldRetry: (error) => remainingRetries > 0 && errors.every((err) => err.name !== error.name),
    onRetry: (error) => {
      errors.push(error);
      remainingRetries -= 1;
    },
...
function onChainQuoteProviderFactory({ getQuoteFunctionName, getQuoterAddress, abi, getCallInputs }) {
  return function createOnChainQuoteProvider({
    ...
    const createGetRoutesWithQuotes = (isExactIn = true) => {
      ...
      return async function getRoutesWithQuote(routes, { blockNumber: blockNumberFromConfig, gasModel, retry: retryOptions }) {
        if (!routes.length) {
          return [];
        }
        const {
        ...
        const multicallConfigs = multicallConfigsOverride?.[chainId] || chunkNYOHPJYN_js.BATCH_MULTICALL_CONFIGS[chainId] || chunkNYOHPJYN_js.BATCH_MULTICALL_CONFIGS[chains.ChainId.ETHEREUM];
        const {
...
var createMixedRouteOnChainQuoteProvider = onChainQuoteProviderFactory({
  getQuoterAddress: (chainId) => chunkNYOHPJYN_js.MIXED_ROUTE_QUOTER_ADDRESSES[chainId],
...
var createV3OnChainQuoteProvider = onChainQuoteProviderFactory({
  getQuoterAddress: (chainId) => chunkNYOHPJYN_js.V3_QUOTER_ADDRESSES[chainId],
...
var schema_exports = {};
chunkNYOHPJYN_js.__export(schema_exports, {
  zPools: () => zPools,
  zRouterGetParams: () => zRouterGetParams,
  zRouterPostParams: () => zRouterPostParams
});
var zChainId = zod.z.nativeEnum(chains.ChainId);
...
var _SwapRouter = class {
  ...
  static encodeV2Swap(trade, options, routerMustCustody, performAggregatedSlippageCheck) {
    ...
    const recipient = routerMustCustody ? chunkNYOHPJYN_js.ADDRESS_THIS : typeof options.recipient === "undefined" ? chunkNYOHPJYN_js.MSG_SENDER : sdk.validateAndParseAddress(options.recipient);
    if (trade.tradeType === sdk.TradeType.EXACT_INPUT) {
    ...
    for (const route of trade.routes) {
      ...
      if (singleHop) {
        ...
        for (let i = 0; i < sections.length; i++) {
          ...
          const recipientAddress = lastSectionInRoute ? recipient : chunkNYOHPJYN_js.ADDRESS_THIS;
  ...
  static encodeSwaps(anyTrade, options, isSwapAndAdd) {
    ...
    const performAggregatedSlippageCheck = sampleTrade.tradeType === sdk.TradeType.EXACT_INPUT && numberOfTrades > 2;
...
Object.defineProperty(exports, 'ADDITIONAL_BASES', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.ADDITIONAL_BASES; }
});
Object.defineProperty(exports, 'ADDRESS_THIS', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.ADDRESS_THIS; }
});
Object.defineProperty(exports, 'BASES_TO_CHECK_TRADES_AGAINST', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.BASES_TO_CHECK_TRADES_AGAINST; }
});
Object.defineProperty(exports, 'BASE_SWAP_COST_STABLE_SWAP', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.BASE_SWAP_COST_STABLE_SWAP; }
});
Object.defineProperty(exports, 'BASE_SWAP_COST_V2', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.BASE_SWAP_COST_V2; }
});
Object.defineProperty(exports, 'BASE_SWAP_COST_V3', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.BASE_SWAP_COST_V3; }
});
Object.defineProperty(exports, 'BATCH_MULTICALL_CONFIGS', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.BATCH_MULTICALL_CONFIGS; }
});
Object.defineProperty(exports, 'BETTER_TRADE_LESS_HOPS_THRESHOLD', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.BETTER_TRADE_LESS_HOPS_THRESHOLD; }
});
Object.defineProperty(exports, 'BIG_INT_TEN', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.BIG_INT_TEN; }
});
Object.defineProperty(exports, 'BIPS_BASE', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.BIPS_BASE; }
});
Object.defineProperty(exports, 'COST_PER_EXTRA_HOP_STABLE_SWAP', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.COST_PER_EXTRA_HOP_STABLE_SWAP; }
});
Object.defineProperty(exports, 'COST_PER_EXTRA_HOP_V2', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.COST_PER_EXTRA_HOP_V2; }
});
Object.defineProperty(exports, 'COST_PER_HOP_V3', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.COST_PER_HOP_V3; }
});
Object.defineProperty(exports, 'COST_PER_INIT_TICK', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.COST_PER_INIT_TICK; }
});
Object.defineProperty(exports, 'COST_PER_UNINIT_TICK', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.COST_PER_UNINIT_TICK; }
});
Object.defineProperty(exports, 'CUSTOM_BASES', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.CUSTOM_BASES; }
});
Object.defineProperty(exports, 'MIN_BNB', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.MIN_BNB; }
});
Object.defineProperty(exports, 'MIXED_ROUTE_QUOTER_ADDRESSES', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.MIXED_ROUTE_QUOTER_ADDRESSES; }
});
Object.defineProperty(exports, 'MSG_SENDER', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.MSG_SENDER; }
});
Object.defineProperty(exports, 'SMART_ROUTER_ADDRESSES', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.SMART_ROUTER_ADDRESSES; }
});
Object.defineProperty(exports, 'STABLE_SWAP_INFO_ADDRESS', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.STABLE_SWAP_INFO_ADDRESS; }
});
Object.defineProperty(exports, 'V2_FEE_PATH_PLACEHOLDER', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.V2_FEE_PATH_PLACEHOLDER; }
});
Object.defineProperty(exports, 'V2_ROUTER_ADDRESS', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.V2_ROUTER_ADDRESS; }
});
Object.defineProperty(exports, 'V3_QUOTER_ADDRESSES', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.V3_QUOTER_ADDRESSES; }
});
Object.defineProperty(exports, 'getStableSwapPools', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.getStableSwapPools; }
});
Object.defineProperty(exports, 'isStableSwapSupported', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.isStableSwapSupported; }
});
Object.defineProperty(exports, 'usdGasTokensByChain', {
  enumerable: true,
  get: function () { return chunkNYOHPJYN_js.usdGasTokensByChain; }
});
