...
class Openocean {
    ...
    getChainInstance(network) {
        if (this._chain === 'ethereum') {
            return ethereum_1.Ethereum.getInstance(network);
        }
        else if (this._chain === 'avalanche') {
            return avalanche_1.Avalanche.getInstance(network);
        }
        else if (this._chain === 'polygon') {
            return polygon_1.Polygon.getInstance(network);
        }
        else if (this._chain === 'harmony') {
            return harmony_1.Harmony.getInstance(network);
        }
        else if (this._chain === 'binance-smart-chain') {
            return binance_smart_chain_1.BinanceSmartChain.getInstance(network);
        }
        else if (this._chain === 'cronos') {
            return cronos_1.Cronos.getInstance(network);
        }
        else {
    ...
    estimateSellTrade(baseToken, quoteToken, amount) {
        return __awaiter(this, void 0, void 0, function* () {
            logger_1.logger.info(`estimateSellTrade getting amounts out baseToken(${baseToken.symbol}): ${baseToken.address} - quoteToken(${quoteToken.symbol}): ${quoteToken.address}.`);
            const reqAmount = new decimal_js_light_1.default(amount.toString())
                ...
                .toNumber();
            logger_1.logger.info(`reqAmount(${baseToken.symbol}):${reqAmount}`);
            ...
            if (quoteRes.status == 200) {
                if (quoteRes.data.code == 200 &&
                    ...
                    logger_1.logger.info(`estimateSellTrade quoteData inAmount(${baseToken.symbol}): ${quoteData.inAmount}, outAmount(${quoteToken.symbol}): ${quoteData.outAmount}`);
    ...
    estimateBuyTrade(quoteToken, baseToken, amount) {
        return __awaiter(this, void 0, void 0, function* () {
            logger_1.logger.info(`estimateBuyTrade getting amounts in quoteToken(${quoteToken.symbol}): ${quoteToken.address} - baseToken(${baseToken.symbol}): ${baseToken.address}.`);
            const reqAmount = new decimal_js_light_1.default(amount.toString())
                .div(new decimal_js_light_1.default((Math.pow(10, baseToken.decimals)).toString()))
                .toNumber();
            ...
            if (quoteRes.status == 200) {
                if (quoteRes.data.code == 200 &&
                    ...
                    const quoteData = quoteRes.data.data;
                    logger_1.logger.info(`estimateBuyTrade reverseData inAmount(${quoteToken.symbol}): ${quoteData.reverseAmount}, outAmount(${baseToken.symbol}): ${quoteData.inAmount}`);
    ...
    executeTrade(wallet, trade, gasPrice, openoceanRouter, ttl, abi, gasLimit, nonce, maxFeePerGas, maxPriorityFeePerGas) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (swapRes.status == 200 && swapRes.data.code == 200) {
                ...
                return this.chainInstance.nonceManager.provideNonce(nonce, wallet.address, (nextNonce) => __awaiter(this, void 0, void 0, function* () {
