...
const near_controllers_1 = require("./near.controllers");
class Near extends near_base_1.NearBase {
    constructor(network) {
        const config = (0, near_config_1.getNearConfig)('near', network);
        super('near', config.network.nodeURL, network, config.network.tokenListSource, config.network.tokenListType, config.manualGasPrice, config.gasLimitTransaction, config_manager_v2_1.ConfigManagerV2.getInstance().get('server.transactionDbPath'));
        ...
        this._gasPriceRefreshInterval =
            config.network.gasPriceRefreshInterval !== undefined
                ...
                : null;
        this.updateGasPrice();
        this.controller = near_controllers_1.NearController;
    }
