...
class BinanceSmartChain extends ethereum_base_1.EthereumBase {
    constructor(network) {
        ...
        super('binance-smart-chain', config.network.chainID, config.network.nodeURL, config.network.tokenListSource, config.network.tokenListType, config.manualGasPrice, config.gasLimitTransaction, config_manager_v2_1.ConfigManagerV2.getInstance().get('server.nonceDbPath'), config_manager_v2_1.ConfigManagerV2.getInstance().get('server.transactionDbPath'));
    ...
    getSpender(reqSpender) {
        ...
        if (reqSpender === 'pancakeswap') {
            ...
            spender = pancakeswap_config_1.PancakeSwapConfig.config.pancakeswapV3NftManagerAddress(this._chain);
        }
        else if (reqSpender === 'sushiswap') {
            spender = sushiswap_config_1.SushiswapConfig.config.sushiswapRouterAddress('binance-smart-chain', this._chain);
        }
        else if (reqSpender === 'openocean') {
            spender = openocean_config_1.OpenoceanConfig.config.routerAddress('binance-smart-chain', this._chain);
        }
        else {
            spender = reqSpender;
        }
