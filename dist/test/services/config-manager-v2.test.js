...
describe('Configuration manager v2 tests', () => {
    ...
    it('reading from config file', (done) => {
        ...
        expect(configManager.get('ethereum.networks.goerli.nativeCurrencySymbol')).toEqual('ETH');
        done();
    });
    ...
    it('writing a valid configuration', (done) => {
        const newKeyPath = 'new-gateway.crt';
        ...
        expect(verifyConfigManager.get('server.certificatePath')).toEqual(newKeyPath);
        expect(verifyConfigManager.get('ethereum.networks.goerli.chainID')).toEqual(970);
    ...
    it('getting namespace objects', (done) => {
        const serverNamespace = configManager.getNamespace('server');
        expect(path_1.default.basename(serverNamespace.schemaPath)).toEqual('server-schema.json');
        expect(path_1.default.dirname(serverNamespace.schemaPath)).toEqual(path_1.default.dirname(config_manager_v2_1.ConfigRootSchemaPath));
        expect(serverNamespace.configurationPath).toEqual(path_1.default.join(tempDirPath, 'test1/server.yml'));
        done();
    });
